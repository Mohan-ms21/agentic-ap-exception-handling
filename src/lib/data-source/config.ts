...
export const dataSourceNameSchema = z.enum(["mock", "n8n", "langgraph"]);
...
export function resolveDataSourceName(
  ...
  const parsed = dataSourceNameSchema.safeParse(value);
  if (!parsed.success) {
    throw new DataSourceConfigError(
      `DATA_SOURCE="${value}" is not valid. Expected one of: ${dataSourceNameSchema.options.join(", ")}.`,
    );
  }
